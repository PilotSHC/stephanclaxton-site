...
The interesting development of the last few years is that this gap is closing. Modern systems modeling, especially SysML v2 but also tools like Modelica and the various scenario description languages, produces models that can be directly executed. The model is the simulation source. The behavior described in the model is the behavior that gets exercised when you run a scenario.
...
The harder shift to make is the cultural one inside the systems engineering team itself. Engineers who built their careers on descriptive modeling, on the craft of producing clear architectural diagrams and well-written requirements documents, often experience the move to executable models as a deskilling.
